 ...
 * 
 * 优势：
 * - 速度快：响应时间0.5-1秒（比通义千问快4倍）
 * - 成本低：¥0.0001/次（比通义千问便宜80%）
 * - 效果好：文本理解能力强
 * 
 * 配置说明：
 * 1. 注册智谱AI：https://open.bigmodel.cn
 * 2. 获取API Key：控制台 → API管理
 * 3. 设置环境变量：ZHIPU_API_KEY
 * 4. 修改配置：llm.provider: zhipu
 * 
 * @author MQ
 * @date 2025-10-26
 */
...
@Slf4j
public class ZhipuLLMClientImpl implements UnifiedLLMClient {
    ...
    public String chat(String prompt, String systemPrompt) {
        try {
            ...
            LLMConfig.ZhipuConfig config = llmConfig.getZhipu();
            log.debug("🚀 [智谱AI] 开始调用，模型: {}", config.getModel());
            // 构建请求
            ...
            if (httpResponse.statusCode() != 200) {
                log.error("❌ [智谱AI] HTTP错误: {}, 响应: {}", 
                        httpResponse.statusCode(), httpResponse.body());
                throw new BusinessException(ErrorCode.SYSTEM_ERROR, 
                        "智谱AI调用失败，状态码: " + httpResponse.statusCode());
            }
            // 解析响应
            LLMResponse response = objectMapper.readValue(httpResponse.body(), LLMResponse.class);
            if (response.getChoices() == null || response.getChoices().isEmpty()) {
                throw new BusinessException(ErrorCode.SYSTEM_ERROR, "智谱AI返回结果为空");
            }
            String content = response.getChoices().get(0).getMessage().getContent();
            long duration = System.currentTimeMillis() - startTime;
            log.info("✅ [智谱AI] 响应成功，耗时: {}ms, tokens: {}, 响应长度: {} 字符", 
                    duration, 
            ...
            log.error("❌ [智谱AI] 调用失败: {}", e.getMessage(), e);
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, 
                    "智谱AI调用失败: " + e.getMessage());
        }
    }
    
    @Override
    public String getProviderName() {
        return "智谱AI (" + llmConfig.getZhipu().getModel() + ")";
    }
    ...
    public boolean isAvailable() {
        try {
            ...
            log.warn("⚠️ [智谱AI] 服务不可用: {}", e.getMessage());
            return false;
        }
    }
}
